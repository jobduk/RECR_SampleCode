...
public class Nav2DController : MonoBehaviour
{
    ...
    public System.Action<Vector3> moveDirStartAction;
    public System.Action<Vector3> moveDirEndAction;
    public System.Action<Vector3> movingDirAction;
    ...
    void Update()
    {
        if (isMoving)
        {
            ...
            {
                dir = (Vector3)pathLeftToGo[0] - transform.position;

                float frameSpeed = speed * Time.deltaTime;
                transform.position += dir.normalized * frameSpeed;
                movingDirAction.Invoke(dir);

                if (((Vector2)transform.position - pathLeftToGo[0]).sqrMagnitude < frameSpeed * frameSpeed)
                {
    ...
    {
        isMoving = false;
        transform.position = pathLeftToGo[pathLeftToGo.Count - 1];
        moveDirEndAction.Invoke(dir);
    }

    public System.Action<Vector3> moveStartAction_TargetPosition;
    ...
    public void GetMoveCommand(Vector2 target)
    {
        ...
        if (pathfinder.GenerateAstarPath(closestNode, targetNode, out path)) //Generate path between two points on grid that are close to the transform position and the assigned target.
        {
            ...
            if (pathLeftToGo.Count > 0)
            {
                isMoving = true;
                dir = (Vector3)pathLeftToGo[0] - transform.position;
                moveDirStartAction.Invoke(dir);
                moveStartAction_TargetPosition.Invoke(GetClosestNode(target));
            }
        }
    }
    ...
    Dictionary<Vector2, float> GetNeighbourNodes2(Vector2 pos)
    {
        Dictionary<Vector2, float> neighbours = new Dictionary<Vector2, float>();
        for (int i = -1; i < 2; i++)
        {
            for (int j = -1; j < 2; j++)
            {
                ...
                Vector2 dir = new Vector2(i, j) * gridSize;
                if (!Physics2D.OverlapPoint(pos + dir, obstacles))
                {
                    neighbours.Add(GetClosestNode(pos + dir), dir.magnitude);
                }
            }
